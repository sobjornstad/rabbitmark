...
class BookmarkTableModel(QAbstractTableModel):
    ...
    def checkEdited(self, mark, content):
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.sm.selectionChanged.connect(self.changeSelection)
        ...
        self.form.searchBox.textChanged.connect(self.changeSearch)
        self.form.tagList.itemSelectionChanged.connect(self.changeSearch)
        self.lastMark = None # the last bookmark we had selected
        #print "before start chain"
        self.changeSearch()
    ...
    def changeSearch(self):
        """
        Change the table and edit view based on a new search.
        """
        print "changeSearch()"
        #oldBlock = self.blockSignals(True) # don't fill the edit pane early
        index = self.tableView.currentIndex()
        if index.row() > 0:
            mark = self.tableModel.getObj(self.tableView.currentIndex())
        else: # '-1' -- indicating no current index
            mark = None
        self.maybeSaveEdited(mark)
        self.fillTable(mark)
        self.fillEditPane()
        #self.blockSignals(oldBlock)

    def changeSelection(self):
        """
        Change the edit view based on a new selection.
        """
    ...
    def maybeSaveEdited(self, mark):
        """
        Pass the last bookmark db object and a representation of what the view
        had in the text boxes to the model. If they differ, the new values will
        be saved in the db object.

        Also, save the current db object to self.lastMark, as by the time this
        method gets called, the selection has already been changed (but not yet
        the text boxes).
        """
        print 'maybeSaveEdited()'
        mRepr = self.mRepr()
        if self.lastMark is not None:
            self.tableModel.checkEdited(self.lastMark, mRepr)
        self.lastMark = mark
